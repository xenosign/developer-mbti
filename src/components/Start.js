...
export default function Start() {
  ...
  async function sqlFetchData() {
    const resCount = await fetch(`http://${HTTPS}/data/count`);
    ...
    const resSurvey = await fetch(`http://${HTTPS}/data/survey`);
    if (resSurvey.status === 200) {
      const surveyData = await resSurvey.json();
      console.log(surveyData);
      const resExplaination = await fetch(`http://${HTTPS}/data/explaination`);
  ...
  async function mongoFetchData() {
    const resMongoCount = await fetch(`http://${HTTPS}/mongo/count`);
    ...
    const resMongoData = await fetch(`http://${HTTPS}/mongo/getdata`);
